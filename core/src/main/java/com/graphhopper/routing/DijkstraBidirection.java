...
public class DijkstraBidirection extends AbstractBidirAlgo
{
            ...
            IntDoubleBinHeap openSet, EdgeWrapper wrapper, EdgeExplorer explorer )
    {
        ...
        boolean backwards = parentRefFrom == parentRefTo;
    ...
    void updateShortest( int nodeId, int ref, double weight )
    {
        ...
        boolean backwards = parentRefFrom == parentRefOther;
        ...
        if ( !backwards )
        {
            ...
            if ( parentRefFrom.getEdgeId(ref) == parentRefOther.getEdgeId(otherRef) )
            {
                return;
            }
            newWeight += turnCostCalc.getTurnCosts(nodeId, parentRefFrom.getEdgeId(ref),
                    parentRefOther.getEdgeId(otherRef));
        } else
        {
            //prevents the shortest path to contain the same edge twice, when turn restriction is around the meeting point
            if ( parentRefTo.getEdgeId(ref) == parentRefOther.getEdgeId(otherRef) )
            {
                return;
            }
            newWeight += turnCostCalc.getTurnCosts(nodeId, parentRefOther.getEdgeId(otherRef),
                    parentRefTo.getEdgeId(ref));
        }
